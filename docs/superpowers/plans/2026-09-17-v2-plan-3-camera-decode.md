...
- Locator (spec §6.2): strict 1:1:3:1:1 row scan plus dotted 7-run at 25 % tolerance; module floor 3 downscaled px; anchored-extent center matching with 3-iteration row/column refinement, centroid fallback; clustering within max(2, module); parallelogram devNorm ≤ 0.35; modules within 2×; side/module 36–75 on the uncorrected module; TL by brightest 3×3 core ≥ 40 luma; cross-product TR/BL; cos-of-folded-rotation module correction.
- Grid size (§3.2): 64 ± 10, mean of four sides.
...
- Drift (§6.5): BFS from the eight corner-adjacent cells; hill-climb over the 3×3 offsets (max 8 steps); widen to the ±2 ring when best Hamming > 20, and always for seeds; clamp ±6.

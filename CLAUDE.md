...
**Cell encoding:** Each cell is `CELL_SIZE=8` pixels. A `floor(frameSize/8) × floor(frameSize/8)` grid minus 36 finder-pattern cells (four 3×3 corner blocks) gives usable cells. Each cell encodes 7 bits (3 = color index into 8 colors; 4 = symbol index into 16 shapes). Supported frame sizes: 128, 192, 256, 384 px.
...
**Camera-specific decode flags (`enableWhiteBalance`, `useRelativeColor`, `symbolThreshold`, `quadrantOffset`, `useHashDetection`, `useLabColor`):** `decodeFramePixels()` accepts optional flags that gate white balance correction, relative color matching, symbol detection sensitivity, corner sample positioning, hash-based symbol detection, and CIELAB color matching. `enableWhiteBalance`, `useRelativeColor`, `useHashDetection`, and `useLabColor` default to `false`; `symbolThreshold` and `quadrantOffset` default to `null` (using the GIF formula and 0.28 respectively). Camera paths set the first five via `DecodeTuningConfig` (defaults: `enableWhiteBalance=true`, `useRelativeColor=true`, `symbolThreshold=0.85`, `quadrantOffset=0.28`, `useHashDetection=true`). `useLabColor` is not directly configurable — it's used as a failover when primary color matching fails the quality gate. GIF decode doesn't need these corrections because pixel colors are exact from the encoder. The flags avoid any regression in the GIF path while improving camera robustness.
...
**White balance finder sampling:** The Von Kries white reference is sampled from the outer corner cells of all four 3×3 finder patterns (grid positions 0,0 / cols-1,0 / 0,rows-1 / cols-1,rows-1), taking per-channel max across all 4 samples. NOT the center cell (1,1) which is dark gray. The center cell has a dark fill with only a tiny white dot — sampling it would produce an incorrect white reference and amplify color errors.
...
**Corner derivation from 4 finder centers (`computeBarcodeCornersFrom4`):** When all 4 finder centers (TL, TR, BL, BR) are available, the x-axis is derived from TL→TR and the y-axis from TL→BL independently, handling non-square trapezoidal distortion where the barcode is not a perfect parallelogram. Each axis unit vector is computed from its respective finder pair, and the 4 barcode corners are derived by stepping `frameSize` along the appropriate axes from each finder center.

**Corner derivation from 2 finder centers (fallback):** The barcode is assumed square. Given TL finder center at grid cell (1,1) and BR at (cols-2, rows-2), the diagonal vector `D = BR - TL` spans `(cols-3)*cs` pixels in each axis. The barcode's x/y unit vectors are: `ux = ((dx+dy)/(2n), (dy-dx)/(2n))`, `uy = (-(dy-dx)/(2n), (dx+dy)/(2n))`. The origin is `TL - 1.5*cs*(ux + uy)`, and the 4 corners are derived by stepping `frameSize` along each axis.
...
**Fallback chain (3 tiers):** (1) 4-point warp via `computeBarcodeCornersFrom4` when all 4 finder centers are available. (2) 2-point warp via `computeBarcodeCorners` when only TL+BR are available (assumes square barcode). (3) Crop+resize when no finder centers are available or warp-based RS decode fails. GIF decode path is unaffected.
...
3. **Vertical confirmation** at each horizontal hit — scans a local window (±3× hSize) around the candidate at `centerX` for a matching vertical bright→dark→bright pattern within 2× of the horizontal width, avoiding interference from colored cells elsewhere in the column
4. **Deduplication** — merges candidates within `imageSize/30` radius (multiple scan lines hit the same finder)
5. **Selection & classification** — scores by hit count + aspect ratio + contrast, classifies as TL/BR/TR/BL by coordinate-sum and coordinate-difference extremes (min x+y → TL, max x+y → BR, max x-y → TR, min x-y → BL)
...
`LocateResult` now includes optional `tlFinderCenter`, `trFinderCenter`, `blFinderCenter`, and `brFinderCenter` (`Point<double>?`) in original image coordinates, present when anchor detection succeeds (null on fallback). These are used by the perspective transform (Priority 2).
...
### ~~RS Block Interleaving~~ ✓ IMPLEMENTED

Both encoder (`web-app/cimbar.js encodeRSFrame`) and decoder (`cimbar_decoder.dart decodeRSFrame`, `web-app/cimbar.js decodeRSFrame`) now use byte-stride interleaving. After RS-encoding N blocks, byte `j` of block `i` is written to output position `j * N + i`. On decode, the inverse permutation reconstructs each block before RS decoding.

**Why:** Without interleaving, all data for block 1 occupies the first ~255 cells in the frame. Camera errors are spatially distributed, so a contiguous region of errors (e.g., from perspective distortion or a finger covering part of the barcode) can exceed the 32-error RS correction capacity for a single block, even when other blocks (containing zero-padded data) decode fine. Interleaving spreads each block's bytes across the entire frame, so 60 contiguous byte errors become ~20 per block across 3 blocks — well within RS capacity.

**Breaking change:** Old GIFs encoded without interleaving will not decode with the new decoder. The test encoder in `android/test/test_utils/cimbar_encoder.dart` is also updated to match.

### LAB Color Space Failover — Android Only ✓ IMPLEMENTED

When the primary color matching (RGB weighted distance or relative color) fails and the RS quality gate rejects the frame (all first 64 bytes are zero), the camera decode paths (`live_scanner.dart`, `camera_decode_pipeline.dart`) retry with CIELAB color matching. The conversion pipeline is sRGB → linear → XYZ (D65) → LAB. Euclidean distance in LAB space is perceptually uniform, better handling camera color shifts from auto-exposure and white balance errors.

The `useLabColor` parameter in `decodeFramePixels` switches color matching to `_nearestColorIdxLab`. LAB matching is only used as a failover — never in the GIF path (exact colors need no correction) and only after RGB/relative matching has already failed.
...
| `test/core/services/frame_locator_test.dart` | Barcode region detection from camera photos: centered barcode, offset barcode, barcode filling entire image, dark image (no barcode), noisy background with scattered bright pixels (verifies anchor detection ignores noise), finder center position validation (detected TL/TR/BL/BR centers within 20px of known positions), and fallback behavior (uniform bright rectangle without finder structure triggers luma-threshold fallback with null finder centers). 7 tests. |
...
| `test/core/services/perspective_transform_test.dart` | Corner derivation from 2 finders (axis-aligned and 30° rotated barcodes produce correct square corners, degenerate inputs return null), `computeBarcodeCornersFrom4` derivation from 4 finders (axis-aligned and rotated, degenerate inputs return null), identity warp preserves content, warp corrects rotated barcode (>90% cell colors match palette with NN sampling), full RS decode round-trip through 2-point perspective warp (256px barcode rotated 10° at 2× in 900px photo), full RS decode round-trip through 4-point perspective warp, fallback path with no finder centers. 9 tests. |
...
- `encodeRSFrame`/`decodeRSFrame` use byte-stride interleaving: byte `j` of block `i` maps to position `j * N + i`. Both JS and Dart implementations must use identical block-size computation (same `while` loop) to ensure N and block sizes match. The interleaving is a no-op when N=1 (frame fits in a single RS block, e.g., 128px frames).

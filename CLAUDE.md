...
The web app encodes and decodes CimBar v2 only. The Android app decodes v2 GIFs via file import; the v2 camera decoder (`FrameDecoder.decode`) is implemented and proven on synthetic degradations, but the camera screens still run v1 until Plan 4. `test-data/goldens/` (golden v2 GIFs plus JSON ground-truth sidecars, generated by `web-app/tools/gen_goldens.js`) is the interoperability contract both sides are tested against. Encryption is optional and unchanged across v1/v2 — unencrypted GIFs can be decoded without a passphrase, and encrypted payloads are auto-detected by their `CB 42 01 00` magic header.

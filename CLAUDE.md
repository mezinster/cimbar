...
- `rateless.js` — GF(256) repair-frame coding for v2.1: non-linear splitmix32+fmix32 coefficient generator (`codingCoefficients` in `format.js`) drives `combineBodies` (XOR/GF-multiply source bodies into one repair body) and `class RatelessAssembler`, which accepts source/repair frames in any order (`add(data, blocksFailed)` → `{accepted, reason}`, reasons include `dependent`/`uncoded`/`duplicate`), tracks `rank`/`total`/`compressed`/`counts`, and reassembles the file once `rank === total` via `framedData()`. Exposes `window.CimbarRateless`
...
- `compress.js` — optional zlib deflate/inflate for the container (spec §4): `maybeDeflate(bytes)` compresses only when it saves at least the configured minimum, else returns the input untouched; `inflateBytes(bytes)` reverses it. Browser: `CompressionStream`/`DecompressionStream('deflate')`; Node (tests/tools): the `zlib` module. Exposes `window.CimbarCompress`
...
- **Compression:** the file container is zlib-deflated before encryption whenever deflate saves at least `compression.minSaving` (5%) of its size; otherwise it is sent unmodified and flag bit 2 stays clear, so already-compressed media costs nothing extra.
...
| `tests/test_compress.js` | `compress.js`: a compressible text file deflates and round-trips through `maybeDeflate`/`inflateBytes`; incompressible random bytes are left alone (below the 5% `compression.minSaving` threshold); `inflateBytes` rejects non-deflate garbage. |

...
| Sharing | `share_handler` | Receive shared files: Android SEND intent, iOS Share Extension → `ShareIntake` (`lib/shared/widgets/share_intake.dart`) → Import tab |
...
- Intent filter accepts shared `image/gif` files (handled by `ShareIntake`)

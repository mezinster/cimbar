...
- `android/pubspec.lock` is now committed, resolved with the pinned Flutter 3.44.8: F-Droid runs `flutter pub get --enforce-lockfile`, and so does CI's *Analyze & Test* job, so a dependency change without a matching lock fails CI rather than an F-Droid build pinned to a release tag.

...
### Added
- **iOS project** (`app/ios/`): bundle id `com.nfcarchiver.cimbar`, home-screen name "CimBar", camera and photo-library permission prompts in all five languages (no microphone), a single opaque 1024×1024 app icon generated from the same mosaic as Android, and a **Share Extension** (`com.nfcarchiver.cimbar.ShareExtension`, App Group `group.com.nfcarchiver.cimbar`) built on `share_handler`. Project edits are made by `app/ios/tool/configure_xcode_project.rb` (the `xcodeproj` gem). CocoaPods only: SPM is disabled in `pubspec.yaml`. CI's new *Build iOS (unsigned)* job compiles the app and extension on macOS and asserts `Runner.app/PlugIns/ShareExtension.appex` is embedded; nothing is signed or distributed yet.
- **Receiving shared GIFs, on Android too.** `share_handler` was a dependency and the manifest declared a `SEND image/gif` filter, but no Dart code read the shared file, so sharing a GIF to the app opened it and dropped the file. `ShareIntake` now selects the first shared file on the Import tab (cold start and while running), and shows a translated message when a shared file can't be read. Sharing a file while a GIF decode is already running shows a message instead of replacing the file out from under it, and sharing while Live Scan or Photo Capture is open closes that screen and shows the Import tab.

...
### Added
- **Web app icon.** The page had no icon; it now has a favicon (SVG plus a 32 px PNG), an Apple touch icon and a web manifest (192/512 icons, name "CimBar"), generated by `tools/gen_store_graphics.js` from the same mosaic as the Android/iOS app icon. The deploy workflow stages them and now uploads each file type with its own Content-Type: it used to force `text/javascript` on every non-HTML file, which would have made browsers reject the icons. `web-app/tests/test_web_icons.js` guards the links, the staging list and the Content-Type table.

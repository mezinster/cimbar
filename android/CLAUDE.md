...
- `payload_decoder.dart` — `decodeFramedPayload(framed, passphrase, {bool compressed = false}) -> ParsedFile`: the shared tail of every v2/v2.1 decode path once frame(s) are fully assembled — strip the u32 length prefix, detect encryption via the `CB 42` magic, decrypt if needed, inflate (`dart:io`'s `ZLibCodec`) if `compressed` — which comes from the assembled frames' header flag, not sniffed from the bytes — then parse the file container. Throws `PassphraseRequiredException` when the payload is encrypted and no passphrase was given, and `FormatException` when inflation fails
